...
module RpcHelper(Conf : sig
  ...
  let bigmap_val_m ?chain ?block bigmap key key_type =
    let cctxt, chain, block = confopts ~chain ~block in
    let hash = Tzutils.Michelson.Handy.hash_expr ~typ:key_type key |> Lwt_main.run in
...
let () =
  ...
  let bridge_prog = get_option_exn (StringOption "-bridge") in
  let endpoint = get_option (StringOption "-endpoint") in
  let base_dir = get_option (StringOption "-base-dir") in
  let bridge = make_bridge bridge_prog in
  let logger =
    if has_flag "-l" then `FullStderr else `Null in
  let cctxt : #Tezos_client_011_PtHangz2.Protocol_client_context.full =
      Tznode.client_context ?endpoint ?base_dir ~logger() |> Lwt_main.run in
  let chain, block =
    let open Bcutils in
  ...
  (match get_option (StringOption "-bigmap") with
   | Some bigmap ->
      ...
      (match get_option_exn (StringOption "-get") |> String.split_on_char ':' with
       | [key; key_type] ->
          bigmap_val (Alpha_context.Big_map.Id.parse_z bigmap) key key_type
          |> print_endline
  ...
  (match get_option (StringOption "-brokers") with
   | Some brokers ->
      ...
      let marshaled = IndexV0.Serialization.to_bytes (Lwt_main.run index) in
