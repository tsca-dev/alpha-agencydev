...
let sign_request ~access_key ~secret_key ~service ~region ~host ~body ~target (meth, uri, headers) =
    ...
    let canonical_headers =
      ...
      ^ "\n"
    in
    let signed_headers = "host;x-amz-content-sha256;x-amz-date;x-amz-target" in
