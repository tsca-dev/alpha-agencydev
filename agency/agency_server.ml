    ...
    let workload() =
      ...
      then (
        ...
        Lwt.catch (fun () ->
            ...
            (fun () ->
              let cctxt, chain, block = create_tezos_context() in
              let index =
...
let supported_network =
  let open Aii_dir.AgencyTypes in
  let chain_id =
    let chain_id, _ =
      Tznode.get_branch ~chain ~block cctxt
      |> Tzutils.strip_tzresult_lwt in
    let tr = Tezos_protocol_006_PsCARTHA.Protocol.Environment.Chain_id.to_b58check in
    tr chain_id in
  { netident = "testnet";
    chain_id; }
...
let handle_agency_request reqd =
  ...
  | "/_tscadev/tmp/indexed" when getting_method meth ->
     ...
     let res =
       sprintf "%d templates:@;%a"
         (List.length templates)
         (List.pp pp_sexp)
         (templates |&> sexp_of_template_entry) in
...
let handle_api_request (path : string) reqd =
  ...
  let rec on_read buf ~off ~len =
    refappend (Bigstringaf.substring buf ~off ~len) read;
    Body.schedule_read reqbody ~on_read ~on_eof
  and on_eof() =
    let open Sexplib in
    ...
    let workload() =
      try
        ...
        Reqd.respond_with_string reqd (Response.create ~headers `OK) resp
      with
      ...
      | ResourceNotFound msg ->
         let msg = "Requested resource not found"^(
             match msg with
             | None -> "" | Some m -> ": "^m ) in
         respond_reqd_with_string ~status:`Not_found reqd msg |> Lwt.return
      | InvalidRequest msg ->
         let msg = "Malformed/Invalid request"^(
             match msg with
             | None -> "" | Some m -> ": "^m ) in
         respond_reqd_with_string ~status:`Bad_request reqd msg |> Lwt.return
      | exn -> 
...
let () =
  ...
  let booksdk = Book_intf.ocaml_booksdk in

  let strip_aresult = function
    | Ok x -> x
    | Error msg -> raise (InvalidRequest (Some msg)) in

  Bridge.register_handler
    Proto0.ProcessGenesisRequest (fun req ->
      if req.network <> supported_network
      then raise (ResourceNotFound (Some ("network: "^req.network.netident)));
      ...
      let book = match Books.books_for_template tmplhash with
        ...
        | h :: _ -> h in
      let (genparam_bytes, initbal) = tmplversion.genspell_interpreter booksdk
        ~spell:req.spell |> strip_aresult in
      ...
      let broker_message = BrokerTypes.GenesisRequest {
            sudo = false;
            genreq = {
                genreq_sprthash = sprthash;
                genreq_tmplid = tznat tmplid;
                genreq_tmplhash = tzbytes raw_tmplhash;
                genreq_genparam = tzbytes genparam_bytes;
                genreq_initbalance = tzmutez initbal;
  
                genreq_broker_fee = tzmutez agency_charge;
                genreq_provider_fee = tzmutez provider_charge;
              };
          } in
      let txn_argument = message_bridge#convert broker_message in
      let txn_amount =
        let (+) = Int64.add in
  ...
  Bridge.register_handler
    Proto0.ProcessInvocationRequest (fun req ->
      ...
      let tmplversion = find_tmplversion tmplhash in
      let rclabel, wparam, amount = tmplversion.invspell_interpreter booksdk
        ~spell:req.spell |> strip_aresult in
  ...
  Bridge.register_handler
    Proto0.InterpretSpiritStatus (fun req ->
      ...
      (match List.find_opt (fun e -> e.silabel = silabel)
       ...
       | None -> Proto0.NoSuchSpiritInterpreter
       | Some intrp ->
          (match ResultWithErrmsg.protect
                   (intrp.interpreter booksdk)
                   spirit
                 |> Result.join with
  ...
  Bridge.register_handler
    Proto0.SimulateGenesis (fun (req, proc) ->
      ...
      | `Succeeded (logs, (watermark, unsigned), _, _, _) ->
