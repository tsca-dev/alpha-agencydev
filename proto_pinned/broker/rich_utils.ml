...
module Hashes = struct
  ...
  let sprthash_prefix_testnet = "\007\024\174\234\127"
  let sprthash_prefix_mainnet = "\007\024\174\236j"

  let calc_sprthash ~testnet ~rawident =
    (* check length of rawident == 7 *)
    (if String.length rawident <> 7
     then invalid_arg "calc_sprthash: rawident must be of length 7");
    let prefix =
      if testnet
      then sprthash_prefix_testnet
      else sprthash_prefix_mainnet in
    let bytes = prefix^rawident in
    bytes |> Base58.safe_encode

  let parse_sprthash' hash =
    let testnet_prefixed, mainnet_prefixed =
      String.(
        starts_with sprthash_prefix_testnet,
        starts_with sprthash_prefix_mainnet) in
    let decoded, testnet =
    match Base58.safe_decode hash with
    | Some b when testnet_prefixed b -> b, true
    | Some b when mainnet_prefixed b -> b, false
    | _ -> invalid_arg "parse_sprthash: invalid sprthash (1)" in
    let rawident = match String.length decoded with
      | 12 -> String.sub decoded 5 7
      | _ -> invalid_arg "parse_sprthash: invalid sprthash (2)" in
    rawident, testnet

  external str_set : string -> int -> char -> unit = "%string_safe_set"

  let make_table ~output prefix varlen totlen =
  ...
  let make_sample prefix totlen =
    let open Tezos_crypto in
    let plen = String.length prefix in
    let buf = String.(prefix^(make (totlen-plen) '0')) in
    let encoded = Base58.safe_encode buf in
    Format.printf "%s : %s@."
      (String.escaped buf) encoded

  let make_sample2 prefix totlen =
    let open Tezos_crypto in
    let plen = String.length prefix in
    let sample c =
